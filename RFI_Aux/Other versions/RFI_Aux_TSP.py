...
def tsp(respondent: str, load_type: str, mw: float) -> float:
    match respondent.lower():
        case 'LCRA':
            if load_type == 'Contract':
                new_mw = mw 
            elif load_type == 'OfficerLetter':
                new_mw = mw * 0.8
        case 'Oncor':
            if load_type == 'Contract':
                new_mw = mw * 0.5326
        ...
        case 'Lonestar':
            if load_type == 'Contract':
                ...
                new_mw = mw * 0.72
        ### Not info on these ones so keeping old percentage ###
        case 'BTU':
            if load_type == 'Contract':
                new_mw = mw * 0.72
            elif load_type == 'OfficerLetter':
        ...
        case 'Rayburn':
            if load_type == 'Contract':
                ...
                new_mw = mw * 0.72
        case 'South Plains Electric Cooperative':
            if load_type == 'Contract':
                new_mw = mw * 0.88
            elif load_type == 'OfficerLetter':
                new_mw = mw * 0.72
...
df = temp_df[['Respondent', 'Load Name', 'Wzone', 'County',
                ...
                'Summer2027', 'Winter2027-2028', 'Summer2028',
                'Summer2030', 'Summer2031', 'Expected Start Year', 'RTP WZ']]
...
for index, row in df.iterrows():
    ...
    rfi_2031 = row['Summer2031']
    rfi_2030 = row['Summer2030']
    rfi_2028 = row['Summer2028']
    rfi_2028min = row['Winter2027-2028']
    rfi_2027 = row['Summer2027']
    ...
    if pd.isna(wz):
        print(f"No weather zone information for {respondent}'s load '{load_name}'")
        continue
    ...
    tsp_2031 = tsp(rfi_2031)
    tsp_2030 = tsp(rfi_2030)
    tsp_2028 = tsp(rfi_2028)
    tsp_2028min = tsp(rfi_2028min)
    tsp_2027 = tsp(rfi_2027)

    mvar2031 = calculate_mvar(tsp_2031, pf)
    mvar2030 = calculate_mvar(tsp_2030, pf)
    mvar2028 = calculate_mvar(tsp_2028, pf)
    mvar2028min = calculate_mvar(tsp_2028min, pf)
    mvar2027 = calculate_mvar(tsp_2027, pf)

    aux_2031 = f"{row['Bus Number']} \"{row['Load ID']}\" \"Closed\" \"YES\" {round(tsp_2031,2)} {mvar2031} \"{row['County']}\" \"{row['Contract/Officer Letter']}\""
    aux_2030 = f"{row['Bus Number']} \"{row['Load ID']}\" \"Closed\" \"YES\" {round(tsp_2030,2)} {mvar2030} \"{row['County']}\" \"{row['Contract/Officer Letter']}\""
    aux_2028 = f"{row['Bus Number']} \"{row['Load ID']}\" \"Closed\" \"YES\" {round(tsp_2028,2)} {mvar2028} \"{row['County']}\" \"{row['Contract/Officer Letter']}\""
    aux_2028min = f"{row['Bus Number']} \"{row['Load ID']}\" \"Closed\" \"YES\" {round(tsp_2028min,2)} {mvar2028min} \"{row['County']}\" \"{row['Contract/Officer Letter']}\""
    aux_2027 = f"{row['Bus Number']} \"{row['Load ID']}\" \"Closed\" \"YES\" {round(tsp_2027,2)} {mvar2027} \"{row['County']}\" \"{row['Contract/Officer Letter']}\""
